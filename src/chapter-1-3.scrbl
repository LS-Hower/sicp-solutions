...
@$${\left| \dfrac{\mathrm{d}f}{\mathrm{d}x} (r) \right| \lessapprox 1}
...
换一个角度，如果放大很多倍，函数 @${f} 在 @${r} 处的图象就会像直线一样了，直线点斜式是 @${y - f(r) = \dfrac{\mathrm{d}f}{\mathrm{d}x} (r) (x - r)} 。我们可以对一次函数作分析，来了解逼近速度。具体地，每迭代一次，距离将会缩短到上一次的 @${\dfrac{\mathrm{d}f}{\mathrm{d}x} (r)} 倍。显然这个数的绝对值应该小于 @${1} 才会让距离越来越近，又一次印证了“导数绝对值小于 @${1} ”这个要求。
...
@$${
 \begin{align*}
   \dfrac{\mathrm{d}f_k(x)}{\mathrm{d}x} &= \dfrac{(2^k-1) + \dfrac{\mathrm{d}f(x)}{\mathrm{d}x}}{2^k}  \\
                                         &= 1 + \dfrac{1}{2^k} \left( \dfrac{\mathrm{d}f(x)}{\mathrm{d}x} - 1 \right)
  \end{align*}
}

因此，我们希望 @${\left| 1 + \dfrac{1}{2^k} \left( \dfrac{\mathrm{d}f}{\mathrm{d}x} (r) - 1 \right) \right| < 1} ，即
