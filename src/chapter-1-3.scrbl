...
说点题外话。用牛顿法求 @${f} 的零点，其实是求 @${x \mapsto x - \dfrac{f(x)}{Df(x)}} 的不动点，这一点原书正文里也有提到。那我们拿刚才有关吸引不动点的结论来分析一下它会如何呢？我们设经过“牛顿变换”的 @${f} 为 @${g} ，即 @${g(x) = x - \dfrac{f(x)}{Df(x)}} 。设 @${f} 的根为 @${r} ，它将是 @${g} 的不动点。我们对 @${g} 求导，得到 @${Dg(x) = \dfrac{g(x) D^{2}g(x)}{(Dg(x))^{2}}} 。这里 @${D^{2}g} 表示 @${g} 的二阶导函数。将 @${r} 代入直接得到 @${0} 了！所以按照刚才过程中所说的，迭代一次之后，和答案的距离就直接变成 @${0} 了吗？事实当然并非如此。刚才过程中所分析的情况中，我们说过，放大很多倍之后，函数图象会是一条直线。准确地说，其实应该是一条“斜线”。函数的行为就像一个一次函数一样，其中斜率不为 @${0} 。而当斜率为 @${0} 时，不能直接认为函数的行为就像一个斜率为 @${0} 的一次函数（常数函数）一样。相反，在牛顿迭代的这个情况下，它的行为会像二次函数一样。当一次项消失时，二次项会成为主导。先前针对一次函数的分析将不再适用，不动点迭代会有不同的行为，比如说：每迭代一次，正确的位数往往会翻倍（这一现象在原书正文脚注 62 中也有提及）；而对于一次函数，每迭代一次，正确的位数只会增加一个常数。
